        ...
        try:
            ...
            if clob_data:
                ...
                for m_poly in markets:
                    ...
                    # ==============================================================
                    try:
                        # Initialize consensus to None for this market
                        consensus = None

                        p_count = m_poly.get('count', 0)
                        p_hours_left = m_poly.get('hours', HEDGE_MAX_TIME_HOURS)
                        p_avg_hist = m_poly.get('daily_avg') or 45.0
                        ...
                        if owned:
                            ...
                            if pos_data:
                                    ...
                                    if trade_key not in executed_trades_this_cycle:
                                        if bid <= MOONSHOT_EXIT_EXPIRED_PRICE:
                                            ...
                                            res = trader.execute(m_poly['title'], b['bucket'], "ROTATE", bid, reason,
                                                                strategy_tag='MOONSHOT', hours_left=p_hours_left,
                                ...
                                # Triggers when loss is extreme AND mathematically impossible to recover
                                    ...
                                    # if profit_pct > PARANOID_TREASURE_MIN_PCT and z_score > PARANOID_TREASURE_Z: should_sell = True; sell_reason = "Paranoid Treasure (Secured)"  # DISABLED: temporalmente desactivado
